    ...
    <section className="relative w-full flex flex-col items-center justify-center flex-wrap">
      ...
      >
        <div className="w-full flex flex-col gap-5 mb-10">
          <Image
            ...
            alt="logo las"
            src={
              acfData?.logo?.node?.sourceUrl ||
              '/images/landingpage/logo-las.svg'
            }
          ...
          <div className="w-full max-w-md font-exo2 text-[#9494A1] text-sm md:text-lg font-light">
            {acfData?.description ? (
              <div dangerouslySetInnerHTML={{ __html: acfData.description }} />
            ) : (
              <>
                Acreditamos que a saúde começa com uma escolha: colocar as
                ...
                <br />
                <br />
                Vamos juntos ativar essa mudança? O futuro da saúde já começou —
                e ele é mais humano, mais conectado e transformador.
              </>
            )}
          </div>
        </div>
        ...
        <Image
          ...
          alt="hero image mask"
          src={
            acfData?.image?.node?.sourceUrl ||
            '/images/landingpage/human-connection-mask.png'
          }
      ...
      >
        <p className="font-exo2 text-black font-semibold text-2xl md:text-4xl text-center">
          {acfData?.bottomTitle || 'Caminhos para transformar a saúde'}
        </p>

        <p className=" font-exo2 text-[#9494A1] font-normal text-sm md:text-lg text-center">
          {acfData?.bottomDescription ||
            'Atuamos em diferentes frentes para gerar impacto — da prevenção ao cuidado, da educação à colaboração.'}
        </p>
      </div>
    </section>
  );
