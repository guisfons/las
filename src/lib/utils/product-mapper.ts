...
export function mapWPProductToProduct(wpProduct: WPProductNode): Product {
  ...
  return {
    ...
    detail: {
      ...
      about: acf?.about?.map(item => item.point).filter((p): p is string => !!p) || [],
      general_information: acf?.generalInformation || "",
      technical_data: acf?.technicalData?.map(row => [
        row.col1 || "",
        row.col2 || "",
        row.col3 || "",
        row.col4 || ""
      ...
      links: acf?.links?.map(link => ({
        title: link.title || "",
        url: link.fileUrl || "",
        file_name: link.fileName || "",
        type: link.type as any || "DEFAULT"
      })) || [],
      videos: acf?.videos?.map(v => ({
        description: v.description || "",
        url: v.videoUrl || ""
      })) || [],
      images: acf?.images?.nodes?.map(img => ({
        url: img.sourceUrl,
        alt: img.altText || ""
      })) || [],
      testimonial: acf?.testimonial ? {
        testimonial: acf.testimonial.testimonialText || "",
        testimonial_pictures: acf.testimonial.testimonialPictures?.nodes?.map(img => img.sourceUrl) || [],
        doctor: {
